...
class ParallelNodeGroup:
  ...
  def __init__(
      ...
      wait_n: int = 1,
      key_reducers: Optional[Dict[str, Union[str, Callable[..., Any]]]] = None,
  ):
    ...
    Args:
        ...
        key_reducers: Per-key merge strategies for parallel state merging.
            Maps state key to a reducer: "append", "sum", "merge", or a
            callable(existing, new) -> merged. Keys without a reducer use
    ...
    self.wait_n = wait_n
    # SECURITY: key_reducers callables run in-process with full access.
    # Only accept from trusted code paths (not user-facing YAML configs).
    self.key_reducers = key_reducers or {}
...
async def execute_parallel_group(
  ...
  with tracer.start_as_current_span("parallel_group_execution") as span:
    ...
    for node_name in group.nodes:
      ...
      # Performance: deep=True ensures branch isolation (no shared mutable refs).
      # Acceptable cost: ctx is small relative to node execution time.
      branch_ctx = ctx.model_copy(deep=True)
    ...
    for node_name in group.nodes:
      ...
      for key, value in branch_data.items():
        ...
        # Apply per-key reducer if configured, otherwise last-write-wins
        reducer = group.key_reducers.get(key)
        if reducer == "append":
          existing = state.data.get(key, [])
          if not isinstance(existing, list):
            existing = [existing]
          state.data[key] = existing + (
              [value] if not isinstance(value, list) else value
          )
        elif reducer == "sum":
          existing = state.data.get(key, 0)
          state.data[key] = existing + value
        elif reducer == "merge" and isinstance(value, dict):
          existing = state.data.get(key, {})
          ...
          state.data[key] = reducer(state.data.get(key), value)
        else:
