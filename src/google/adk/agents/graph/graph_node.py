...
class GraphNode:
  ...
  def __init__(
    ...
    Args:
        ...
        reducer: Strategy for merging output into state (auto-defaults to MERGE
            for function nodes, OVERWRITE for agent nodes)
    ...
    # Auto-default: function nodes use MERGE, agent nodes use OVERWRITE.
    if (
        reducer == StateReducer.OVERWRITE
        and function is not None
        and agent is None
    ):
      self.reducer = StateReducer.MERGE
      logger.debug(
          "Node '%s': function node auto-defaults to MERGE reducer."
          " Dict returns spread into state.data as flat keys (same"
          " convention as output_key). Pass reducer= to override.",
          name,
      )
    else:
  ...
  def _default_output_mapper(
      self, output: Any, state: GraphState
  ) -> GraphState:
    ...
    if self.reducer == StateReducer.MERGE:
      # MERGE: dict returns are spread into state.data as flat keys
      # (same convention as output_key). Non-dict falls back to OVERWRITE.
      if isinstance(output, dict):
        new_state.data.update(output)
      else:
        new_state.data[self.name] = output
    elif self.reducer == StateReducer.OVERWRITE:
      new_state.data[self.name] = output
      ...
      if output is None:
        # Cannot sum with None output — preserve existing value
        pass
      else:
        if existing is None:
          # Infer zero-value from output type: "" for str, 0 for int/float
