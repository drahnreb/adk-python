...
class InterruptReasonerConfig:
  ...
  include_state_in_prompt: bool = (
      False  # opt-in — state may contain PII/secrets
  )
  max_state_size: int = 10000
  fallback_action: str = "pause"  # fail-safe — don't auto-continue on LLM error

  # Fail-safe defaults follow ADK's Tool Confirmation pattern:
  # - TC defaults to "don't execute" on non-confirmation
  # - InterruptReasoner defaults to "pause" on LLM error
  # - State not included by default (may contain PII/secrets)
