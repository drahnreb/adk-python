...
class CheckpointService:
  ...
  async def create_checkpoint(
    ...
    async with session_lock:
      ...
      async with self._traced("create", checkpoint_id, session.id) as _:
        ...
        # Calculate state size only when validation is enabled
        state_size = None
        if self.config.max_state_size_bytes > 0:
          from .utils import PydanticJSONEncoder

          state_json = json.dumps(
              state_snapshot, cls=PydanticJSONEncoder
          ).encode("utf-8")
  ...
  async def restore_checkpoint(
    ...
    async with self._traced("restore", checkpoint_id, session.id) as _:
      ...
      if restore_state:
        snapshot_keys = set(metadata.state_snapshot.keys())
        # Delete stale keys not in checkpoint (proper deletion, not None)
        stale_keys = [
            k
            for k in list(session.state.keys())
            if not k.startswith("_") and k not in snapshot_keys
        ]
        for k in stale_keys:
          del session.state[k]
        # Then apply snapshot via event (existing code)
        restore_event = Event(
