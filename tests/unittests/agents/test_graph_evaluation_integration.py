...
class StatefulAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx):
        ...
        yield Event(
            author=self.name,
            content=types.Content(parts=[types.Part(text=json.dumps(self._state_updates))]),
        )
...
async def test_graph_path_extraction_from_intermediate_data():
    ...
    invocation = Invocation(
        userContent=user_content or types.Content(parts=[types.Part(text="test")]),
        finalResponse=final_response or types.Content(parts=[types.Part(text="done")]),
    ...
    assert result.overall_score == 1.0, f"Score: {result.overall_score}, Expected: 1.0"
...
async def test_node_execution_count_extraction_from_intermediate_data():
    ...
    graph.set_start("n1")
    graph.add_edge("n1", "n2")
    graph.add_edge("n2", "n1", condition=lambda s: s.metadata.get("iteration", 0) < 2)
    graph.set_end("n1")
    ...
    assert result.overall_score >= 0.0, "Should have extracted counts from intermediate_data"
    assert len(result.per_invocation_results) == 1
...
async def test_graph_metadata_event_format():
    ...
    assert "[GraphMetadata]" in text, "Metadata event should have [GraphMetadata] marker"
...
async def test_state_extraction_from_intermediate_data():
    ...
    graph = GraphAgent(name="test_graph")
    agent1 = StatefulAgent(name="agent1", state_updates={"count": 1, "status": "processing"})
    agent2 = StatefulAgent(name="agent2", state_updates={"count": 2, "status": "done"})
