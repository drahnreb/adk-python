...
from google.adk.agents.graph.interrupt import InterruptMode, InterruptConfig
...
async def test_immediate_cancellation_saves_state():
    ...
    assert session.state.get("graph_cancelled") == True, "graph_cancelled flag not set"
    assert "graph_cancelled_at_node" in session.state, "graph_cancelled_at_node not saved"
    assert session.state.get("graph_can_resume") == True, "graph_can_resume flag not set"

    # Verify partial execution state was saved
    assert "graph_iteration" in session.state
    ...
    assert cancelled_at == "node2", f"Should be cancelled at node2, got {cancelled_at}"
...
async def test_cancellation_unregisters_session():
    ...
    assert interrupt_service.is_active(session.id) == False  # Cancelled means not active
