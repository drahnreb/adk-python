...
async def _collect_events(
    graph: GraphAgent, ctx: InvocationContext
) -> list[Event]:
...
class TestResumeFromSavedNode:
  ...
  async def test_resume_from_saved_node(self):
    ...
    graph = _build_linear_graph(
        "g", [agent_a, agent_b, agent_c], ["nA", "nB", "nC"]
    )
...
class TestStateEventGuards:
  ...
  async def test_non_resumable_context_no_end_of_agent(self):
    ...
    state_events = [
        e for e in events if e.actions and e.actions.agent_state is not None
    ]
  ...
  async def test_resumable_context_emits_state_events(self):
    ...
    state_events = [
        ...
        for e in events
        if e.actions
        and (e.actions.agent_state is not None or e.actions.end_of_agent)
    ]
    assert len(state_events) > 0
...
class TestPauseOnLongRunningTool:
  ...
  async def test_pause_on_long_running_tool(self):
    ...
    pause_events = [e for e in events if e.long_running_tool_ids]
  ...
  async def test_resume_after_pause_continues(self):
    ...
    end_events = [e for e in events2 if e.actions and e.actions.end_of_agent]
