...
from google.adk.checkpoints.checkpoint_service import (
    CheckpointService,
...
async def test_full_interrupt_workflow_continue():
    ...
    graph = GraphAgent(
        ...
        interrupt_config=InterruptConfig(mode=InterruptMode.AFTER, reasoner=reasoner),
    )

    node_a = GraphNode(name="node_a", agent=MockAgent(name="agent_a", response="output_a"))
    node_b = GraphNode(name="node_b", agent=MockAgent(name="agent_b", response="output_b"))
    ...
    await interrupt_service.send_message("test_session", "Check output", action="check")
...
async def test_full_interrupt_workflow_rerun():
    ...
    # Verify interrupt decision was recorded
...
async def test_multi_node_interrupt_sequence():
    ...
    await interrupt_service.send_message("test_session", "Check node_a", action="continue")
    await interrupt_service.send_message("test_session", "Check node_b", action="continue")
    await interrupt_service.send_message("test_session", "Check node_c", action="continue")
...
async def test_interrupt_timing_before():
    ...
    interrupt_service.register_session("test_session")
    await interrupt_service.send_message("test_session", "Pre-check", action="continue")
...
async def test_interrupt_timing_both():
    ...
    await interrupt_service.send_message("test_session", "Pre-check", action="continue")
    await interrupt_service.send_message("test_session", "Post-check", action="continue")
...
async def test_interrupt_action_pause_resume():
    ...
    await interrupt_service.send_message("test_session", "Check output", action="continue")
...
async def test_state_preservation_during_interrupt():
    ...
    node_a = GraphNode(name="node_a", agent=MockAgent(name="agent_a", response="output"))
    ...
    interrupt_service.register_session("test_session")
    await interrupt_service.send_message("test_session", "Check state", action="continue")
...
async def test_checkpoint_integration_with_interrupts():
    ...
    interrupt_service.register_session("test_session")
    await interrupt_service.send_message("test_session", "Check before checkpoint", action="continue")
...
async def test_error_recovery_via_interrupt():
    ...
    error_agent = ErrorAgent(name="error_agent", error_message="Recoverable error")
    node_a = GraphNode(name="node_a", agent=error_agent)
    node_b = GraphNode(name="node_b", agent=MockAgent(name="agent_b", response="recovered"))
...
async def test_immediate_cancel_stops_execution():
    ...
    node_a = GraphNode(name="node_a", agent=SlowMockAgent(name="slow_agent", event_count=10))
    node_b = GraphNode(name="node_b", agent=MockAgent(name="agent_b"))
...
async def test_interrupt_with_conditional_routing():
    ...
    # Conditional routing
    graph.add_edge("node_a", "node_b", condition=lambda state: state.data.get("go_b", True))
    graph.add_edge("node_a", "node_c", condition=lambda state: state.data.get("go_c", False))
    ...
    interrupt_service.register_session("test_session")
    await interrupt_service.send_message("test_session", "Check routing", action="continue")
...
async def test_interrupt_action_skip():
    ...
    await interrupt_service.send_message("test_session", "Skip node_a", action="skip")
...
async def test_concurrent_interrupts_different_sessions():
    ...
    for i in range(3):
        ...
        await interrupt_service.send_message(session_id, f"Message {i}", action="continue")
...
async def test_interrupt_state_isolation():
    ...
    await interrupt_service.send_message("session_1", "Message for session 1", action="action_1")
    await interrupt_service.send_message("session_2", "Message for session 2", action="action_2")
