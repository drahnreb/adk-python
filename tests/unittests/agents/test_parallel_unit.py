...
class StateModifyingAgent(BaseAgent):
    ...
    async def run_async(self, ctx):
        ...
        yield Event(
            author=self.name,
            content=types.Content(parts=[types.Part(text=f"Set {self.key}={self.value}")]),
        )
...
class TestCollectEvents:
    ...
    async def test_collect_events_success(self):
        ...
        async def event_generator():
            yield Event(author="test", content=types.Content(parts=[types.Part(text="event1")]))
            yield Event(author="test", content=types.Content(parts=[types.Part(text="event2")]))
...
class TestExecuteParallelGroupJoinStrategies:
    ...
    async def test_wait_all_strategy(self):
        ...
        nodes = {
            "a": GraphNode(name="a", agent=SimpleAgent(name="agent_a", output="out_a")),
            "b": GraphNode(name="b", agent=SimpleAgent(name="agent_b", output="out_b")),
            "c": GraphNode(name="c", agent=SimpleAgent(name="agent_c", output="out_c")),
        }
        ...
        events = []
        async for event in execute_parallel_group(group, nodes, state, ctx, mock_execute_node):
            events.append(event)
    ...
    async def test_wait_any_strategy(self):
        ...
        nodes = {
            "fast": GraphNode(name="fast", agent=SimpleAgent(name="fast", output="fast_out", delay=0.001)),
            "slow": GraphNode(name="slow", agent=SimpleAgent(name="slow", output="slow_out", delay=10.0)),
        }
    ...
    async def test_wait_n_strategy(self):
        ...
        nodes = {
            chr(97 + i): GraphNode(name=chr(97 + i), agent=SimpleAgent(name=f"agent_{chr(97 + i)}", output=f"out_{chr(97 + i)}"))
            for i in range(5)
        }
...
class TestExecuteParallelGroupErrorPolicies:
    ...
    async def test_fail_fast_policy_cancels_others(self):
        ...
        nodes = {
            "good": GraphNode(name="good", agent=SimpleAgent(name="good", output="success", delay=10.0)),
            "bad": GraphNode(name="bad", agent=ErrorProneAgent(name="bad", error_msg="fail_fast_error")),
        }
        ...
        with pytest.raises(RuntimeError, match="fail_fast_error"):
            async for _ in execute_parallel_group(group, nodes, state, ctx, mock_execute_node):
                pass
    ...
    async def test_continue_policy_continues_on_error(self):
        ...
        nodes = {
            "good1": GraphNode(name="good1", agent=SimpleAgent(name="good1", output="success1")),
            "bad": GraphNode(name="bad", agent=ErrorProneAgent(name="bad")),
            "good2": GraphNode(name="good2", agent=SimpleAgent(name="good2", output="success2")),
        }
    ...
    async def test_collect_policy_collects_all_errors(self):
        ...
        nodes = {
            "bad1": GraphNode(name="bad1", agent=ErrorProneAgent(name="bad1", error_msg="error1")),
            "bad2": GraphNode(name="bad2", agent=ErrorProneAgent(name="bad2", error_msg="error2")),
        }
...
class TestStateIsolationAndMerging:
    ...
    async def test_state_isolation_with_deepcopy(self):
        ...
        class StateCheckingAgent(BaseAgent):
            ...
            async def run_async(self, ctx):
                ...
                yield Event(
                    author=self.name,
                    content=types.Content(parts=[types.Part(text=f"Saw: {initial_value}")]),
                )
        ...
        state = GraphState(data=initial_data.copy(), metadata={})
        ctx = create_mock_context()
    ...
    async def test_state_merge_conflict_detection(self):
        ...
        nodes = {
            "a": GraphNode(name="a", agent=SimpleAgent(name="agent_a", output="a_output")),
            "b": GraphNode(name="b", agent=SimpleAgent(name="agent_b", output="b_output")),
        }
...
class TestTaskLookupAndCancellation:
    ...
    async def test_cancellation_of_pending_tasks_wait_any(self):
        ...
        nodes = {
            "fast": GraphNode(name="fast", agent=SimpleAgent(name="fast", delay=0.001)),
            "slow1": GraphNode(name="slow1", agent=SimpleAgent(name="slow1", delay=10.0)),
            "slow2": GraphNode(name="slow2", agent=SimpleAgent(name="slow2", delay=10.0)),
        }
...
class TestEdgeCases:
    ...
    async def test_multiple_events_per_node(self):
        ...
        nodes = {
            "multi": GraphNode(name="multi", agent=MultiEventAgent(name="multi", num_events=5)),
        }
    ...
    @pytest.mark.asyncio
    async def test_metadata_merge_conflicts(self):
        """Test metadata merge also detects conflicts."""
        group = ParallelNodeGroup(
            nodes=["a", "b"],
            join_strategy=JoinStrategy.WAIT_ALL,
        )
        ...
        async def mock_execute_node(node, branch_state, ctx):
            # Simulate both branches modifying same metadata key
            branch_state.metadata["conflict_meta"] = f"meta_from_{node.name}"
        ...
        with patch("google.adk.agents.graph.parallel.logger") as mock_logger:
            ...
            # Should log metadata conflict warning
            assert any(
                "Metadata merge conflict detected" in str(call)
                for call in mock_logger.warning.call_args_list
            )
