    ...
    async def _run_async_impl(self, ctx):
        ...
        yield Event(
            ...
            content=types.Content(
                parts=[types.Part(text=f"Set {self._state_key}={self._state_value}")]
            )
        )
...
async def test_rewind_to_node_basic(session_service):
    ...
    graph.add_node(GraphNode(name="step1", agent=SimpleAgent("agent1", "Output 1")))
    graph.add_node(GraphNode(name="step2", agent=SimpleAgent("agent2", "Output 2")))
    graph.add_node(GraphNode(name="step3", agent=SimpleAgent("agent3", "Output 3")))
    ...
    # Execute graph
    runner = Runner(app_name="test_app", agent=graph, session_service=session_service, auto_create_session=True)
    ...
    # Get session
    session = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s1")
    ...
    await graph.rewind_to_node(session_service, "test_app", "u1", "s1", "step2")

    # Verify we can still access the session after rewind
    session_after_rewind = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s1")
...
async def test_rewind_to_node_in_loop(session_service):
    ...
    graph.add_edge(
        "validator",
        "counter",
        condition=lambda s: s.metadata.get("iteration", 0) < 3,
    )
    ...
    new_message = types.Content(parts=[types.Part(text="Start")])
    async for event in runner.run_async(user_id="u1", session_id="s2", new_message=new_message):
        pass

    # Get session
    session = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s2")
    ...
    if len(counter_invocations) >= 2:
        ...
        await graph.rewind_to_node(session_service, "test_app", "u1", "s2", "counter", invocation_index=1)

        # Verify session still accessible
        session_after = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s2")
...
async def test_rewind_restores_state(session_service):
    ...
    graph = GraphAgent(name="test_graph")
    graph.add_node(GraphNode(name="step_a", agent=SimpleAgent("agent_a", "Output A")))
    graph.add_node(GraphNode(name="step_b", agent=SimpleAgent("agent_b", "Output B")))
    graph.add_node(GraphNode(name="step_c", agent=SimpleAgent("agent_c", "Output C")))
    ...
    new_message = types.Content(parts=[types.Part(text="Start")])
    async for event in runner.run_async(user_id="u1", session_id="s3", new_message=new_message):
        pass

    # Get session after full execution
    session = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s3")
    ...
    await graph.rewind_to_node(session_service, "test_app", "u1", "s3", "step_b")

    # Verify session still accessible after rewind
    session_after = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s3")
    assert session_after is not None
...
async def test_rewind_invalid_node(session_service):
    ...
    new_message = types.Content(parts=[types.Part(text="Start")])
    async for event in runner.run_async(user_id="u1", session_id="s4", new_message=new_message):
        pass

    session = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s4")

    # Try to rewind to non-existent node
    with pytest.raises(ValueError, match="has not been executed yet"):
        await graph.rewind_to_node(session_service, "test_app", "u1", "s4", "nonexistent_node")
...
async def test_rewind_invalid_invocation_index(session_service):
    ...
    new_message = types.Content(parts=[types.Part(text="Start")])
    async for event in runner.run_async(user_id="u1", session_id="s5", new_message=new_message):
        pass

    session = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s5")

    # Try to rewind with invalid index
    with pytest.raises(ValueError, match="out of range"):
        await graph.rewind_to_node(session_service, "test_app", "u1", "s5", "step1", invocation_index=10)
...
async def test_rewind_preserves_path(session_service):
    ...
    new_message = types.Content(parts=[types.Part(text="Start")])
    async for event in runner.run_async(user_id="u1", session_id="s6", new_message=new_message):
        pass

    session = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s6")

    # Path should be [a, b, c]
    # Note: Path tracking depends on implementation details

    # Rewind to b - verify it works
    await graph.rewind_to_node(session_service, "test_app", "u1", "s6", "b")

    # Verify session still accessible
    session_after = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s6")
    assert session_after is not None
...
async def test_rewind_with_branching(session_service):
    ...
    graph = GraphAgent(name="test_graph")
    graph.add_node(GraphNode(name="start", agent=SimpleAgent("start_agent", "Start")))
    ...
    new_message = types.Content(parts=[types.Part(text="Start")])
    async for event in runner.run_async(user_id="u1", session_id="s7", new_message=new_message):
        pass

    session = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s7")
    ...
    await graph.rewind_to_node(session_service, "test_app", "u1", "s7", "start")

    # Verify session still accessible
    session_after = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s7")
    assert session_after is not None
...
async def test_rewind_negative_index(session_service):
    ...
    graph.add_edge(
        "repeater",
        "repeater",
        condition=lambda s: s.metadata.get("iteration", 0) < 2,
    )
    ...
    new_message = types.Content(parts=[types.Part(text="Start")])
    async for event in runner.run_async(user_id="u1", session_id="s8", new_message=new_message):
        pass

    session = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s8")
    ...
    if len(repeater_invocations) >= 2:
        ...
        await graph.rewind_to_node(session_service, "test_app", "u1", "s8", "repeater", invocation_index=-1)

        # Verify session still accessible
        session_after = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s8")
        assert session_after is not None
...
async def test_rewind_session_state_integration(session_service):
    ...
    new_message = types.Content(parts=[types.Part(text="Start")])
    async for event in runner.run_async(user_id="u1", session_id="s9", new_message=new_message):
        pass

    session = await session_service.get_session(app_name="test_app", user_id="u1", session_id="s9")

    # Verify node_invocations in session state
    assert "node_invocations" in session.state
    assert isinstance(session.state["node_invocations"], dict)

    # Verify invocation IDs are tracked
    node_invocations = session.state["node_invocations"]
...
async def test_rewind_empty_node_invocations(session_service):
    ...
    with pytest.raises(ValueError, match="has not been executed yet"):
        await graph.rewind_to_node(session_service, "test_app", "u1", "s10", "step1")
