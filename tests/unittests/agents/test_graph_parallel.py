...
async def test_parallel_group_already_executed(session_service):
    ...
    assert event_texts.count("merged") == 1, f"Expected merge to execute once, got {event_texts.count('merged')} times"
...
async def test_parallel_wait_n_validation(session_service):
    ...
    with pytest.raises(ValueError, match="cannot be greater than number of nodes"):
        ParallelNodeGroup(
            nodes=["agent1", "agent2"],
            join_strategy=JoinStrategy.WAIT_N,
            wait_n=3,
