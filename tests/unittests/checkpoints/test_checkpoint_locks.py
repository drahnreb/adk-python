...
@pytest.mark.asyncio
async def test_concurrent_checkpoint_creation_no_race(checkpoint_service, session):
    ...
    assert len(limit_reached) == 5, f"Expected 5 limit errors, got {len(limit_reached)}"
...
@pytest.mark.asyncio
async def test_sequential_checkpoints_no_contention(checkpoint_service, session):
...
async def test_lock_wait_time_telemetry(checkpoint_service, session):
    """Test that lock wait time is tracked (basic smoke test)."""
