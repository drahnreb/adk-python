...
@pytest.mark.asyncio
async def test_restore_checkpoint_deletes_stale_keys():
  """Restoring a checkpoint properly deletes keys not in the snapshot."""
  from google.adk.sessions.in_memory_session_service import InMemorySessionService
  ...
  # Create checkpoint with state {"a": 1, "b": 2}
  ...
  # Verify stale key "c" is properly deleted (not set to None)
  result = await session_service.get_session(
      app_name="test_app", user_id="test_user", session_id=fresh.id
  )
  assert (
      "c" not in result.state
  ), "Stale key 'c' should be deleted, not set to None"
