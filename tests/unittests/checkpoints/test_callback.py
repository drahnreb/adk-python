...
class TestCheckpointCallback:
  ...
  async def test_before_agent_creates_checkpoint(
    ...
    # Checkpoint should be created (counter=1 for first call)
    checkpoint_id = f"{session.id}-{simple_agent.name}-1-before"
  ...
  async def test_after_agent_creates_checkpoint(
    ...
    # Checkpoint should be created (counter=1 for first call)
    checkpoint_id = f"{session.id}-{simple_agent.name}-1-after"
    checkpoint = await checkpoint_service.get_checkpoint(
        session=session,
        checkpoint_id=checkpoint_id,
    )
  ...
  async def test_checkpoint_before_only(
    ...
    # Before should create checkpoint (counter=1)
    await callback.before_agent(callback_context)
    before_id = f"{session.id}-{simple_agent.name}-1-before"
    before_cp = await checkpoint_service.get_checkpoint(session, before_id)
    assert before_cp is not None

    # After should NOT create checkpoint (checkpoint_after=False)
    await callback.after_agent(callback_context)
    after_id = f"{session.id}-{simple_agent.name}-2-after"
  ...
  async def test_checkpoint_after_only(
    ...
    # Before should NOT create checkpoint (checkpoint_before=False)
    await callback.before_agent(callback_context)
    before_id = f"{session.id}-{simple_agent.name}-1-before"
    # Should raise CheckpointNotFoundError (P0.4 fix)
    from google.adk.checkpoints.models import CheckpointNotFoundError
    ...
    # After should create checkpoint (counter=1)
    await callback.after_agent(callback_context)
    after_id = f"{session.id}-{simple_agent.name}-1-after"
  ...
  async def test_multiple_agents_same_session(
    ...
    # Both checkpoints should exist (counter=1 for agent1, counter=2 for agent2)
    cp1 = await checkpoint_service.get_checkpoint(
        session, f"{session.id}-agent1-1-before"
    )
    cp2 = await checkpoint_service.get_checkpoint(
        session, f"{session.id}-agent2-2-before"
    )
