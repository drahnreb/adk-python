...
@pytest.mark.asyncio
async def test_delta_chain_broken_with_intermediate_missing(checkpoint_service, session):
...
@pytest.mark.asyncio
async def test_restore_checkpoint_delta_chain_broken(checkpoint_service, session):
...
@pytest.mark.asyncio
async def test_create_checkpoint_with_corrupted_base(checkpoint_service, session):
    ...
    session.state[f"_checkpoint_{checkpoint1.checkpoint_id}"] = {"invalid": "data"}
...
@pytest.mark.asyncio
async def test_delta_reconstruction_without_reconstruct_flag(checkpoint_service, session):
