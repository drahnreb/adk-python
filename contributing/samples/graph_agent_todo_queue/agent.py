...
def _get_classifier(state: GraphState) -> TodoClassification:
  """Parse classifier output (may be a JSON string or a dict)."""
  val = state.data.get("classifier", {})
  if isinstance(val, str):
    try:
      return TodoClassification.model_validate_json(val)
    except (ValueError, TypeError):
      return TodoClassification(
          todo_id="", todo_type="", priority=5, has_more=False
      )
  if isinstance(val, dict):
    try:
      return TodoClassification.model_validate(val)
    except (ValueError, TypeError):
      return TodoClassification(
          todo_id="", todo_type="", priority=5, has_more=False
      )
...
def _is_data_task(state: GraphState) -> bool:
  return _get_classifier(state).todo_type == "data_processing"


def _is_notification_task(state: GraphState) -> bool:
  return _get_classifier(state).todo_type == "notification"


def _is_cleanup_task(state: GraphState) -> bool:
  return _get_classifier(state).todo_type == "cleanup"


def _has_more_items(state: GraphState) -> bool:
  return _get_classifier(state).has_more is True
...
def build_todo_queue_graph(
  ...
  classifier = LlmAgent(
      ...
      instruction=(
          ...
          " Determine its type (data_processing, notification, or cleanup),"
          " priority (1-5), and whether there are more unprocessed items"
...
root_agent = build_todo_queue_graph(InMemorySessionService())
