...
class DataFetchAgent(BaseAgent):
    ...
    def __init__(self, name: str, data_source: str, delay_ms: int = 100, **kwargs):
    ...
    async def _run_async_impl(self, ctx):
        ...
        yield Event(
            ...
            content=types.Content(
                parts=[
                    types.Part(
                        text=f"✅ Fetched {len(data['records'])} records from {self._data_source}"
                    )
                ]
            ),
        )
...
async def scenario_1_parallel_wait_all():
    ...
    fetch_users = DataFetchAgent(name="fetch_users", data_source="users_db", delay_ms=150)
    fetch_products = DataFetchAgent(
        name="fetch_products", data_source="products_db", delay_ms=100
    )
    fetch_orders = DataFetchAgent(name="fetch_orders", data_source="orders_db", delay_ms=200)
    ...
    graph.add_node(GraphNode(name="aggregate", agent=aggregate))
...
async def scenario_2_parallel_wait_any():
    ...
    validate = ValidationAgent(name="validate")
    fetch_cache = DataFetchAgent(name="fetch_cache", data_source="cache", delay_ms=50)
    fetch_db = DataFetchAgent(name="fetch_db", data_source="database", delay_ms=150)
    ...
    graph.add_node(GraphNode(name="validate", agent=validate))
    graph.add_node(GraphNode(name="fetch_cache", agent=fetch_cache))
    graph.add_node(GraphNode(name="fetch_db", agent=fetch_db))
    graph.add_node(GraphNode(name="fetch_api", agent=fetch_api))
    graph.add_node(GraphNode(name="transform", agent=transform))
...
async def scenario_3_parallel_wait_n():
    ...
    graph.add_node(GraphNode(name="validate", agent=validate))
    graph.add_node(GraphNode(name="model1", agent=model1))
    graph.add_node(GraphNode(name="model2", agent=model2))
    graph.add_node(GraphNode(name="model3", agent=model3))
    graph.add_node(GraphNode(name="aggregate", agent=aggregate))
...
async def scenario_4_rewind_with_parallel():
    ...
    # Create agents
    validate = ValidationAgent(name="validate")
    fetch_users = DataFetchAgent(name="fetch_users", data_source="users", delay_ms=50)
    fetch_products = DataFetchAgent(
    ...
    print("\n✅ Scenario 4 complete")
    print("   Note: Rewind works with parallel groups - all branches re-executed!")
...
async def scenario_6_interrupts_during_parallel():
    ...
    graph.add_node(GraphNode(name="fetch1", agent=fetch1))
    graph.add_node(GraphNode(name="fetch2", agent=fetch2))
    graph.add_node(GraphNode(name="aggregate", agent=aggregate))
    ...
    print("\n✅ Scenario 6 complete")
    print("   Note: In production, you could send interrupts via InterruptService")
...
async def scenario_7_state_isolation():
    ...
    graph.add_node(GraphNode(name="branch1", agent=branch1))
    graph.add_node(GraphNode(name="branch2", agent=branch2))
    graph.add_node(GraphNode(name="check", agent=check))
...
async def main():
    ...
    print(
        "║"
        + "  GraphAgent Parallel Execution & Rewind Features - Comprehensive Demo".center(
            78
        )
        + "║"
    )
    ...
    try:
        ...
        print("=" * 80)
        print("\nKey Takeaways:")
        print("1. Parallel execution works with WAIT_ALL, WAIT_ANY, WAIT_N strategies")
        print("2. Rewind integration works - can rewind to nodes that trigger parallel groups")
        ...
        print("  A: YES! Rewind restores to before node execution, re-runs parallel group")
        print("- Q: What about session state communication?")
        print("  A: Branches are isolated during execution, merged after completion")
