...
class DataFetchAgent(BaseAgent):
  ...
  def __init__(
    ...
    super().__init__(
        name=name, data_source=data_source, delay_ms=delay_ms, **kwargs
    )
  ...
  async def _run_async_impl(self, ctx):
    ...
    data = {
        ...
        "records": [f"{self.data_source}_record_{i}" for i in range(3)],
...
class TransformAgent(BaseAgent):
  ...
  def __init__(self, name: str, transformation: str, **kwargs):
    super().__init__(name=name, transformation=transformation, **kwargs)
  ...
  async def _run_async_impl(self, ctx):
    ...
    yield Event(
        ...
        content=types.Content(
            parts=[
                types.Part(
                    text=f"✅ Applied transformation: {self.transformation}"
                )
            ]
        ),
    )
...
async def scenario_7_state_isolation():
  ...
  class StateModifyingAgent(BaseAgent):
    ...
    def __init__(self, name: str, key: str, value: str, **kwargs):
      super().__init__(name=name, key=key, value=value, **kwargs)
...
def build_root_graph() -> GraphAgent:
  """Build a default graph for ADK CLI discovery."""
  graph = GraphAgent(name="parallel_features_root")
  graph.add_node("validate", agent=ValidationAgent(name="validate"))
  graph.add_node(
      "fetch_users",
      agent=DataFetchAgent(name="fetch_users", data_source="users_db"),
  )
  graph.add_node(
      "fetch_products",
      agent=DataFetchAgent(name="fetch_products", data_source="products_db"),
  )
  graph.add_node(
      "aggregate",
      agent=AggregationAgent(name="aggregate"),
  )

  graph.add_parallel_group(
      "fetch_group",
      ParallelNodeGroup(
          nodes=["fetch_users", "fetch_products"],
