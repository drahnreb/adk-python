...
class StageAgent(BaseAgent):
  ...
  def __init__(self, name: str, stage_type: str, **kwargs):
    super().__init__(name=name, stage_type=stage_type, **kwargs)
...
def build_review_stage_graph(
  ...
  graph.add_node(
      "execute",
      agent=execute_agent,
      output_mapper=lambda output, s: GraphState(
          data={**s.data, "stage_content": output}
      ),
  )
