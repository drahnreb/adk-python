...
async def dynamic_task_dispatcher(
  ...
  """
  task_queue = deque(state.data.get("task_queue", []))

  if not task_queue:
    print("✅ Task queue empty - all tasks complete!")
    return {"all_complete": True, "tasks_remaining": 0}

  # Pop next task — O(1) via deque.popleft() vs O(N) list.pop(0)
  ...
  # Update state (convert deque back to list for JSON-serializable state)
  state.data["task_queue"] = list(task_queue)
