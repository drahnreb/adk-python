...
def _create_agents():
  ...
  _analyzer = LlmAgent(
      ...
      model=_MODEL,
      instruction=(
          "You are a risk analyzer. Analyze the input task and assess its"
          " risk level. Set checkpoint_requested=true ONLY when risk_level"
...
async def run_scenario(
  ...
  assessment = AnalysisOutput(
      finding="", risk_level="?", justification="", checkpoint_requested=False
  )
  report_text: str = ""
  ...
  async for event in runner.run_async(
    ...
    if event.author == "analyzer":
      try:
        assessment = AnalysisOutput.model_validate_json(text)
      ...
      requested = assessment.checkpoint_requested
      flag = " [CHECKPOINT REQUESTED]" if requested else ""
      print(f"  Analyzer: risk={risk}{flag}")
      print(f"    {assessment.justification[:100]}")
...
root_agent = build_agent_checkpoint_graph(InMemorySessionService())
