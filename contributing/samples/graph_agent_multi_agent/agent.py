...
def build_multi_agent_graph() -> GraphAgent:
  coordinator = LlmAgent(
      ...
      model=_MODEL,
      instruction=(
          "You are a research coordinator. Given a research topic, split it"
          " into exactly two independent subtopics for parallel investigation."
          " Output each subtopic on its own line prefixed with 'SUBTOPIC A:'"
          " and 'SUBTOPIC B:'."
      ),
  ...
  critic = LlmAgent(
      ...
      instruction=(
          ...
          "completeness, and accuracy. Set decision to 'approve' if good, "
          "or 'revise' if it needs work. Always provide feedback."
      ),
...
async def main() -> None:
  ...
  async for event in runner.run_async(
    ...
    if author == "coordinator":
      ...
      # output_schema=ReviewResult guarantees valid structured JSON
      review = ReviewResult.model_validate_json(text.strip())
      print(f"Critic verdict: {review.decision.upper()}")
...
root_agent = build_multi_agent_graph()
