...
class PeerReview(BaseModel):
  """Structured output for peer reviewer."""
  ...
  return (
      LlmAgent(
          ...
          instruction=(
              ...
              " key papers related to the given topic. Include title, authors,"
              " and key findings for each paper."
          ),
      ),
      LlmAgent(
          ...
          instruction=(
              ...
              " review, propose 3-5 testable hypotheses with rationale for"
              " each."
          ),
      ),
      LlmAgent(
          ...
          instruction=(
              ...
              " methods to test the hypotheses. Cover participants,"
      ...
      LlmAgent(
          ...
          instruction=(
              ...
              " Provide scores (1-10) for each criterion and specific"
...
def analyze_results(state: GraphState, ctx) -> Dict[str, Any]:
  ...
  Returns:
      Dict merged into state.data via StateReducer.MERGE (default for function nodes)
  """
...
def needs_revision(state: GraphState) -> bool:
  ...
  review = state.get_parsed("peer_review", PeerReview)
  if review is None:
    return False
  avg_score = sum(review.scores.values()) / max(len(review.scores), 1)
...
_root_session_service = InMemorySessionService()
_root_checkpoint_service = CheckpointService(
    session_service=_root_session_service
)
_root_interrupt_service = InterruptService()
root_agent = build_research_workflow(
    _root_session_service,
    _root_checkpoint_service,
    _root_interrupt_service,
)
