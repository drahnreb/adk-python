...
from google.adk.agents.graph.callbacks import (
    NodeCallbackContext,
...
async def research_observability_callback(ctx: NodeCallbackContext) -> Optional[Event]:
    ...
    progress = (current_iteration / ctx.invocation_context.agent.max_iterations) * 100
    ...
    parts = [
        ...
        # Progress indicator
        types.Part(text=f"Progress: {progress:.1f}% (iteration {current_iteration})"),
        ...
        # Execution metadata
        types.Part(
            text=f"**Metadata**: Agent={ctx.node.agent.name if ctx.node.agent else 'function'}, "
...
literature_agent = LlmAgent(
    ...
    instruction=(
        "You are a research literature reviewer. "
        "Search for and summarize key papers related to the given topic. "
        "Output a JSON list of papers with title, authors, and key findings."
    ),
)
...
hypothesis_agent = LlmAgent(
    ...
    instruction=(
        ...
        "Based on the literature review, propose 3-5 testable hypotheses. "
        "Output a JSON list of hypotheses with rationale."
    ),
)

# 3. Methodology Designer Agent
methodology_agent = LlmAgent(
    name="methodology_designer",
    model="gemini-2.0-flash-exp",
    instruction=(
        "You are a research methodology expert. "
        "Design experimental methods to test the hypotheses. "
        "Output a JSON dict with methodology sections: participants, materials, procedure."
    ),
)

# 4. Results Analyzer Agent (Simulated)
def analyze_results(state: GraphState) -> GraphState:
    """Simulate data analysis (in real scenario, this would run experiments)."""
    # Simulate analysis results
    results = {
        "hypothesis_1": {"supported": True, "p_value": 0.023, "effect_size": 0.42},
        "hypothesis_2": {"supported": False, "p_value": 0.156, "effect_size": 0.18},
        "hypothesis_3": {"supported": True, "p_value": 0.001, "effect_size": 0.67},
    }
...
paper_writer_agent = LlmAgent(
    name="paper_writer",
    model="gemini-2.0-flash-exp",
    instruction=(
        "You are an academic paper writer. "
        "Write a research paper with sections: Abstract, Introduction, Methods, Results, Discussion. "
        "Use the literature review, hypotheses, methodology, and results from the state. "
        "Output well-structured academic prose."
    ),
)

# 6. Peer Reviewer Agent (Quality Check)
peer_reviewer_agent = LlmAgent(
    name="peer_reviewer",
    model="gemini-2.0-flash-exp",
    instruction=(
        "You are a peer reviewer. "
        "Review the paper for: clarity, scientific rigor, statistical validity, and writing quality. "
        "Provide a review with scores (1-10) and specific suggestions for improvement. "
        "Output JSON with scores and comments."
    ),
)
...
def needs_revision(state: GraphState) -> bool:
    ...
    avg_score = sum(review.get("scores", {}).values()) / max(len(review.get("scores", {})), 1)
    return avg_score < 7.0  # Needs revision if average score < 7/10


def revision_count_ok(state: GraphState) -> bool:
    """Check if we've exceeded max revisions."""
    return state.metadata.get("revision_count", 0) < 3
...
def build_research_workflow(
    ...
    interrupt_reasoner = InterruptReasoner(
        config=InterruptReasonerConfig(
            ...
            instruction=(
                "You are an interrupt reasoning agent for a research paper writing workflow. "
                "Analyze interrupt messages from researchers and decide the best action. "
                "Consider: Is the feedback about quality? Should we rerun with guidance? "
                "Should we pause for human review? Should we defer the feedback for later?"
            ),
        )
    )
    ...
    graph = GraphAgent(
        ...
        description="Advanced research paper writing workflow with interrupt & observability",
        max_iterations=20,
        checkpointing=True,
    ...
    # If review is good or max revisions reached, finish
    graph.add_edge(
        "peer_review",
        "END",
        condition=lambda s: not needs_revision(s) or not revision_count_ok(s),
    )
...
async def scenario_1_basic_execution():
    ...
    graph = build_research_workflow(session_service, checkpoint_service, interrupt_service)
    ...
    ctx = graph.create_invocation_context(
        session=session,
        session_service=session_service,
        user_content=types.Content(
            parts=[types.Part(text="Research topic: Impact of AI on software development")]
        ),
    )

    # Run workflow
    print("Running research workflow...\n")
    async for event in graph.run_async(ctx):
    ...
    print("\n✅ Workflow completed!")
    print(f"Final state keys: {list(session.state.get('graph_state', {}).get('data', {}).keys())}")
...
async def scenario_2_interrupt_with_reasoning():
    ...
    async def send_interrupt_after_delay():
        ...
        await interrupt_service.send_interrupt(
            ...
            text="The literature review missed key papers on neural architecture search. Please include them.",
    ...
    print(f"\n📊 LLM Decision: {decision.get('action')} - {decision.get('reasoning')}")
...
async def scenario_3_checkpointing_and_resume():
    ...
    for cp in checkpoints:
        print(f"   - {cp.checkpoint_id}: {cp.metadata.get('graph_node', 'unknown')}")
...
async def scenario_4_immediate_cancellation():
    ...
    print(f"   - Cancelled at node: {session.state.get('graph_cancelled_at_node', 'unknown')}")
    print(f"   - Can resume: {session.state.get('graph_can_resume', False)}")
    print(f"   - Partial state saved: {bool(session.state.get('graph_state'))}")

    # Show partial state
    if session.state.get("graph_state"):
        partial_state = session.state["graph_state"].get("data", {})
...
async def scenario_5_all_interrupt_timings():
    ...
    ctx = graph_before.create_invocation_context(
        session=session_before,
        session_service=session_service,
        user_content=types.Content(parts=[types.Part(text="Write a paper on AI safety")]),
    )

    async for event in graph_before.run_async(ctx):
        pass

    await interrupt_task

    print(f"✅ BEFORE mode test complete. Peer review skipped: {not bool(session_before.state.get('peer_review'))}\n")
...
async def main():
    ...
    print("\n" + "🎯" * 40)
    print("GraphAgent Advanced Examples - All Features")
    print("🎯" * 40)
