...
async def _planner_after_callback(ctx: NodeCallbackContext):
  """Extend graph topology after planner runs.

  Reads the planner's structured output and adds discovered steps
  as new nodes/edges. This runs between planner completion and
  the next-node routing decision — the right time for extension.
  """
  if ctx.node.name != "planner":
    return None

  graph = ctx.invocation_context.agent
  raw = ctx.state.data.get("planner")

  # Parse planner output (dict from auto-parse or JSON string)
  if isinstance(raw, dict):
    try:
      design = PipelineDesign.model_validate(raw)
    except (ValueError, TypeError):
      return None
  elif isinstance(raw, str):
    try:
      design = PipelineDesign.model_validate_json(raw)
    except (ValueError, TypeError):
      return None
  else:
    return None
  ...
  # Only extend once (idempotent check)
  first_step = f"step_{design.steps[0].replace(' ', '_').replace('-', '_')}"
  if first_step in graph.nodes:
    return None

  # Extend graph with discovered steps
  await extend_graph_with_steps(graph, design.steps)
  return None


def _make_after_callback(checkpoint_cb):
  """Compose planner topology extension with checkpoint callback."""

  async def _combined(ctx: NodeCallbackContext):
    await _planner_after_callback(ctx)
    if checkpoint_cb:
      await checkpoint_cb.after_node(ctx)
...
def build_base_graph(
  ...
  planner = LlmAgent(
      ...
      instruction=(
          ...
          " identify 2-4 distinct processing steps required"
          " (e.g., 'validate', 'transform', 'enrich', 'aggregate', 'export')."
      ),
  ...
  graph = GraphAgent(
      ...
      max_iterations=20,
      after_node_callback=_make_after_callback(checkpoint_callback),
  )

  graph.add_node("planner", agent=planner)
  graph.set_start("planner")
  # No edges or end node yet — _planner_after_callback adds them dynamically
...
async def main() -> None:
  ...
  async for event in runner.run_async(
    ...
    if author == "planner":
      # _planner_after_callback auto-extends the graph topology
      raw = event.content.parts[0].text or ""
      try:
        design = PipelineDesign.model_validate_json(raw)
      except (ValueError, TypeError):
        design = PipelineDesign(steps=[], description="")
      print(f"Planner designed pipeline: {design.description}")
      print(f"Steps discovered: {design.steps}\n")
