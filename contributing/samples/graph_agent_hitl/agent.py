...
async def run_with_approval(
  ...
  last_assessment = RiskAssessment(
      action="", risk_level="unknown", justification=""
  )
  last_execution: str = ""
  ...
  async for event in runner.run_async(
    ...
    if author == "analyzer":
      ...
      try:
        last_assessment = RiskAssessment.model_validate_json(text)
      except (ValueError, TypeError):
        last_assessment = RiskAssessment(
            action="", risk_level="unknown", justification=""
        )
      risk = last_assessment.risk_level.upper()
      action = last_assessment.action
  ...
  print(f"\nRisk level: {last_assessment.risk_level}")
  print(
...
_root_graph, _root_interrupt_service = build_hitl_graph(
    InMemorySessionService()
)
root_agent = _root_graph
